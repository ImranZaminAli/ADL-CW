...
parser.add_argument("--learning-rate", default=3e-1, type=float, help="Learning rate")
...
def main(args):
    ...
    model = CNN(args=args, dropout=dropout)
...
class CNN(nn.Module):
    def __init__(self, args, dropout):
        ...
        self.initialise_sigmoid_layer(self.fc2)
    ...
    @staticmethod
    def initialise_sigmoid_layer(layer):
        if hasattr(layer, "bias"):
            nn.init.zeros_(layer.bias)
        if hasattr(layer, "weight"):
...
class Trainer:
    def __init__(
        ...
        self.step = 0
        self.scheduler = torch.optim.lr_scheduler.ExponentialLR(optimizer=self.optimizer, gamma=0.93)
